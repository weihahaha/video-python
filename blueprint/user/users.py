...
def register():
    ...
    data: dict = MapVer(request.get_json(), 
                        ...
                        Bakedin(key='email', max=150, required=True),
                        Bakedin(key='code', max=8, required=True),
                        Bakedin(key='iconbase64', max=100*1024, required=True))
...
# 修改头像
@userApp.route('/changeicon', methods=['GET'])
def changeIcon():
    
    iconBase64 = StrVer(data=request.args.get('iconbase64'), max=100*1024, min=10)
    userId = StrVer(data=request.args.get('id'), max=100, min=10)
    data = usersDb.find_one({'pid': userId}, {'pid': 1})
    if data is  None:
        abort(makeResponse(-1, '用户不存在'))
    usersIcon.update_one({'pid': userId}, {'$set': {'iconbase64': iconBase64}})
...
def userInfo():
    userId = StrVer(data=request.args.get('id'), max=100, min=10)
    data = usersDb.find_one({'pid': userId}, {'pid': 1, 'name': 1})
    if data is  None:
        abort(makeResponse(-1, '用户不存在'))

    icon = usersIcon.find_one({'pid': userId})
    data['icon'] = icon
...
def userMeinfo():
    ...
    Pperations = userVideoPperation.find_one({'pid': userId}, {"likes": 1, 'favorites': 1})
